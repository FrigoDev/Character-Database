...
import { fetchCommentsByPostId,fetchAllUsers,createComment } from "./crudAPI.js";
...
document.addEventListener('onupdate', async() => {
    ...
    const [userComments,users] =await Promise.all([fetchCommentsByPostId(post.id),fetchAllUsers()]);
    ...
    const handleOnSutmit=async(e)=>{
        ...
        commentsSection.innerHTML += GenComment({...newComment,user:users.find((user)=>user.id==author)});
        form.reset();
    }
    commentForm.addEventListener('submit',handleOnSutmit);
    commentForm.querySelector("select").innerHTML=users.map((user)=>`<option value="${user.id}">${`${user.name} ${user.lastName}`}</option>`).join("\n");

    const  reStructureUserComments = userComments.map((comment)=>{
        const user = users.find((user)=>user.id==comment.user);
        return {...comment,user};
    })
    commentsSection.innerHTML= reStructureUserComments.map((comment)=>GenComment(comment)).join("\n");
})